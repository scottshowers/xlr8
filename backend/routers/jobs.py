...
async def get_jobs(limit: int = 50):
    ...
    logger.warning(f"[JOBS] GET /jobs called with limit={limit}")
    try:
        jobs = ProcessingJobModel.get_all(limit=limit)
        logger.warning(f"[JOBS] Got {len(jobs)} jobs from database")
        ...
        for idx, job in enumerate(jobs):
            ...
            # Debug - log raw and parsed for first 3 jobs
            if idx < 3:
                job_id_short = job.get('id', 'none')[:8] if job.get('id') else 'none'
                logger.warning(f"[JOBS] Job {job_id_short}: raw input_data type={type(job.get('input_data'))}, value={job.get('input_data')}")
                logger.warning(f"[JOBS] Job {job_id_short}: parsed input_data={input_data}")
            ...
            if idx < 3:
                logger.warning(f"[JOBS] Job {job_id_short}: FINAL filename={filename}")
