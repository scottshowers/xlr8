...
async def list_jobs(
    ...
    """
    List all processing jobs with full details.
    
    Returns jobs from Supabase processing_jobs table with ability to cancel.
    """
    try:
        supabase = get_supabase()
        if not supabase:
        ...
        query = supabase.table("processing_jobs").select(
            "id, job_type, status, created_at, updated_at, completed_at, project_id, filename, error_message"
        ...
        for job in (result.data or []):
            ...
            if created and completed:
                try:
                    ...
                    duration = int((e - c).total_seconds())
                except:
                    pass
            
            # Determine if stuck (processing > 15 min)
            is_stuck = False
            if job.get('status') in ['processing', 'queued'] and created:
                try:
                    from datetime import datetime, timezone
                    c = datetime.fromisoformat(created.replace('Z', '+00:00'))
                    age_minutes = (datetime.now(timezone.utc) - c).total_seconds() / 60
                    is_stuck = age_minutes > 15
                except:
                    pass
            ...
            jobs.append({
                ...
                "can_cancel": job.get('status') in ['queued', 'processing']
            })
        
        # Summary stats
        active = len([j for j in jobs if j['status'] in ['queued', 'processing']])
        stuck = len([j for j in jobs if j.get('is_stuck')])
        ...
        logger.error(f"[JOBS] List jobs error: {e}")
        raise HTTPException(500, str(e))
...
async def cancel_job(job_id: str):
    """
    Cancel/kill a processing job.
    
    Sets status to 'cancelled' so it won't be retried.
    """
    try:
        supabase = get_supabase()
        if not supabase:
        ...
        if job.data.get('status') not in ['queued', 'processing']:
            raise HTTPException(400, f"Job is already {job.data.get('status')} - cannot cancel")
        
        # Update status to cancelled
        from datetime import datetime
        result = supabase.table("processing_jobs").update({
            "status": "cancelled",
        ...
        logger.error(f"[JOBS] Cancel job error: {e}")
        raise HTTPException(500, str(e))
...
async def cleanup_stuck_jobs(
    ...
    Cancels any jobs that have been in 'processing' or 'queued' status
    for longer than max_age_minutes.
    """
    try:
        supabase = get_supabase()
        if not supabase:
        ...
        # Find stuck jobs
        stuck = supabase.table("processing_jobs").select("id, status, created_at").in_(
        ...
        for job in (stuck.data or []):
            try:
                ...
                cancelled += 1
                logger.info(f"[JOBS] Auto-cancelled stuck job {job['id']}")
            except Exception as e:
                logger.warning(f"[JOBS] Failed to cancel {job['id']}: {e}")
        
        return {
            "success": True,
            "jobs_found": len(stuck.data or []),
