...
import { Tooltip, PageHeader } from '../components/ui';
    ...
    <div style={{ 
      ...
      <div style={{ padding: '1rem' }}>
        ...
        {(truthType === 'reality' || truthType === 'configuration') && targetScope === 'project' && (
          <div style={{ marginBottom: '1rem' }}>
            <Tooltip title="Domain Classification" detail="Domains help organize data by business area. Auto-detect uses AI to classify based on column names." action="Select or create custom domain">
              <label style={{ 
                fontSize: '0.7rem', fontWeight: 600, color: c.textMuted, 
                textTransform: 'uppercase', letterSpacing: '0.05em', 
                display: 'block', marginBottom: '0.5rem', cursor: 'help'
              }}>
                Domain (optional)
              </label>
            </Tooltip>
            ...
            <Tooltip title="Custom Domain" detail="Create a domain for specialized data types not covered by built-in options. Add signal keywords to help auto-detection." action="Good for industry-specific data">
              <button
        ...
        {truthType === 'reference' && targetScope === 'global' && (
          <div style={{ marginBottom: '1rem' }}>
            <Tooltip title="System Tag" detail="Tag vendor documents with their HCM/ERP system. Projects using this system will see these docs automatically." action="UKG, Workday, ADP, etc.">
              <label style={{ 
                fontSize: '0.7rem', fontWeight: 600, color: c.textMuted, 
                textTransform: 'uppercase', letterSpacing: '0.05em', 
                display: 'block', marginBottom: '0.5rem', cursor: 'help'
        ...
        >
          ...
          <p style={{ fontWeight: 600, color: c.text, margin: '0 0 0.25rem', fontSize: '0.9rem', textAlign: 'center' }}>
            Drop files or click to browse
          </p>
          <p style={{ fontSize: '0.75rem', color: c.textMuted, margin: 0, textAlign: 'center' }}>PDF, Excel, Word, CSV</p>
        </div>
...
function FilesPanel({ c, project, targetScope }) {
  ...
  const structuredFiles = (structuredData?.files || []).filter(f => {
    // Filter out 0-row files (often from PDF extraction with no structured data)
    if (f.type === 'duckdb' && (f.total_rows === 0 || f.rows === 0)) {
      return false;
    }
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        background: isGlobalScope ? `${c.accent}10` : `${c.primary}10`,
        border: `1px solid ${isGlobalScope ? c.accent : c.primary}30`,
        borderRadius: 10,
      }}>
        {isGlobalScope ? (
          <>
            <BookOpen size={20} style={{ color: c.accent }} />
            <div>
              <div style={{ fontWeight: 600, color: c.text, fontSize: '0.9rem' }}>Reference Library</div>
              <div style={{ fontSize: '0.75rem', color: c.textMuted }}>Global documents shared across all projects</div>
            </div>
          </>
        ) : (
          <>
            ...
            <div>
              ...
              <div style={{ fontSize: '0.75rem', color: c.textMuted }}>
                {project?.customer ? `${project.customer} • ` : ''}Files specific to this project
              </div>
            </div>
          </>
        )}
