...
class SecurePayrollParser:
    ...
    def parse_pdf(
        ...
        try:
            # Strategy 1: Camelot (best for complex tables with lines)
            ...
            # Strategy 2: pdfplumber (if Camelot found few/no tables)
            if not result['success'] or len(result['tables']) < 2:
                ...
                try:
                    ...
                    self.logger.warning(f"pdfplumber failed: {str(e)}, continuing to next strategy...")
    ...
    def _table_to_dataframe(self, table: List[List]) -> pd.DataFrame:
        """
        Convert table (list of lists) to pandas DataFrame with robust column handling
        ...
        for i, h in enumerate(headers):
            if h is None or str(h).strip() == '':
                ...
                # Remove newlines, extra whitespace, special chars
