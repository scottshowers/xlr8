...
async def unified_chat(request: UnifiedChatRequest):
    ...
    try:
        # Get customer_id (UUID) from project name for RAG filtering
        customer_id = None
        ...
        if project and SUPABASE_AVAILABLE:
            try:
                ...
                if result.data:
                    ...
                    logger.info(f"[UNIFIED] Resolved customer_id: {customer_id}, product: {product_id}")
            except Exception as e:
        ...
        if session['engine']:
            ...
            engine = IntelligenceEngineV2(project or 'default', customer_id=customer_id, product_id=product_id)
