                ...
                try:
                    ...
                    if result['success']:
                        ...
                        with col1:
                            ...
                            if st.button("📥 Generate Excel", use_container_width=True, key="gen_excel"):
                                try:
                                    with st.spinner("Creating Excel file..."):
                                        ...
                                        st.session_state.export_buffer = output_buffer.getvalue()
                                        st.session_state.export_ready = True
                                        st.success("✅ Excel file generated! Click download below.")
                                        st.rerun()
                            ...
                            if st.session_state.export_ready and st.session_state.export_buffer:
                                st.download_button(
                        ...
                        with col3:
                            # Raw data for manual adjustment
                            if st.button("🔧 Download for Manual Editing", use_container_width=True, key="gen_raw"):
                                try:
                                    with st.spinner("Creating editable CSV..."):
                                        if len(result['tables']) > 0:
                                            # Export all tables separately
                                            import zipfile
...
4. Save the files
5. Use Data Analysis tab to upload modified files

Note: Column names in first row - keep them for re-import
"""
