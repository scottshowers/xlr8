  ...
  const resetPreferences = async (resetType = 'all') => {
    try {
      const response = await api.post('/chat/unified/reset-preferences', {
        ...
        customer_id: customerId,  // UUID - this is what intents are stored under
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <button
            ...
            style={{
              ...
              background: colors.primaryLight || '#e8f5e9',
              border: `1px solid ${colors.primary || '#83b16d'}`,
              borderRadius: 6,
              cursor: 'pointer',
              color: colors.primary || '#83b16d',
              fontSize: '0.75rem',
              fontWeight: 500,
              display: 'flex',
              alignItems: 'center',
