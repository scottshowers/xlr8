...
interface AnalysisResult {
  ...
  statistics: {
    ...
    rcr_employees: number;
    high_priority: number;
    low_priority: number;
    monitor: number;
    avg_rcr_wages: number;
    highest_wages: number;
  };
  download_url: string;
  message: string;
}
      ...
      <div className="max-w-4xl mx-auto px-6 py-10">
        {!result ? (
          <div className="space-y-6">
            ...
            >
              {loading ? <><Loader2 className="w-5 h-5 animate-spin" />Analyzing...</> : <><TrendingUp className="w-5 h-5" />Run Analysis</>}
            </button>
          </div>
        ) : (
          <div className="space-y-6">
