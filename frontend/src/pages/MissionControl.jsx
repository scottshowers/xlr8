 ...
 * 
 * WIRED TO REAL API - Shows ALL projects sorted by most recent activity.
 * 
...
const MissionControl = () => {
  ...
  const attentionItems = [
    ...
    ...projects.filter(p => p.hasRealData && (!p.playbooks || p.playbooks.length === 0)).map(p => ({
      ...
      meta: `${p.tablesAnalyzed} tables analyzed`,
      projectId: p.id
    })),
  ].slice(0, 5);

  // Recent projects - sort by updated_at DESC, include ALL
  const recentProjects = [...projects]
    ...
    .slice(0, 6) // Show 6 to fit nicely
    .map(p => ({
      id: p.id,
  ...
  if (loading) {
    return <div className="page-loading"><Loader2 size={24} className="spin" /><p>Loading dashboard...</p></div>;
  }
            ...
            <div key={project.id} className="project-card" onClick={() => navigate(`/projects/${project.id}/hub`)}>
              ...
              <div className="project-card__stats">
                {project.hasRealData ? (
                  ...
                  <>
                    <div className="project-stat">
                      <div className={`project-stat__value ${project.pending > 5 ? 'project-stat__value--critical' : 'project-stat__value--warning'}`}>{project.pending}</div>
